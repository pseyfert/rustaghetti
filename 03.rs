...
fn check_intersect(x: isize, y: isize, line: &Line) -> bool {
    ...
    while step_success(&mut iterable_line) {
        if iterable_line.x == x && iterable_line.y == y {
            return true
        }
    }
    return false;
...
fn meet(line1: &Line, line2: &Line) -> Vec<Point> {
    let mut iterable_line1 = create_iterable_line(line1);
    let mut points: Vec<Point> = Vec::new();

    while step_success(&mut iterable_line1) {
        if check_intersect(iterable_line1.x, iterable_line1.y, line2) {
            points.push(Point{x: iterable_line1.x, y: iterable_line1.y});
        }
    }
